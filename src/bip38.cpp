...
void ComputePreFactor(std::string strPassphrase, std::string strSalt, uint256& prefactor)
{
    ...
    scrypt(strPassphrase.c_str(), strPassphrase.size(), BEGIN(s), strSalt.size() / 2, BEGIN(prefactor), 16384, 8, 8, 32);
}
...
void ComputeSeedBPass(CPubKey passpoint, std::string strAddressHash, std::string strOwnerSalt, uint512& seedBPass)
{
    ...
    uint256 s2(salt);
    scrypt(BEGIN(passpoint), HexStr(passpoint).size() / 2, BEGIN(s2), salt.size() / 2, BEGIN(seedBPass), 1024, 1, 1, 64);
}
...
//    CBigNum bnFactorB(factorB);
//    CBigNum bnPriv = bnFactorB.mul_mod();

    //double check that the address hash matches our final privkey
    CKey k;
    k.SetPrivKey(static_cast<const unsigned char*>(privKey.begin()), privKey.size());
